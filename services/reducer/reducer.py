...
RESULT_TOPIC = os.getenv("INPUT_TOPIC", "ai.results.v2")
GROUP_ID = os.getenv("CONSUMER_GROUP", "reducer-group")
...
for msg in consumer:
    try:
        ...
        # 4. FINALIZE (ВНУТРИ ЦИКЛА!)
        if status == "running" and total == done:
            cur.execute(
                """
                UPDATE ai_jobs
                SET status = 'done'
        ...
        print(f"[reducer] ERROR job={job_id}: {e}", flush=True)
        raise
